...
void __fastcall TFormMain::UpdateMetaSprite(void)
{
	...
	ImageMetaSprite->Picture->Bitmap->Canvas->Brush->Color=(TColor)outPalette[bgPal[palBank*16+0]];
...
void __fastcall TFormMain::FormCreate(TObject *Sender)
{
	...
	memset(bgPal,0x0f,sizeof(bgPal));
	...
	if(Shift.Contains(ssLeft))
	{
		if(n>=0)
		{
			if(n&0x100)
			{
				...
				col=bgPal[palBank*16+palActive*4+0];
...
void __fastcall TFormMain::MCHRSwapColorsClick(TObject *Sender)
{
	...
	if(FormSwapColors->Swap)
	{
		...
		if(FormSwapColors->RemapPalette)
		{
			...
			bgPal[palBank*16+1*4+0]=bgPal[palBank*16+0];
			bgPal[palBank*16+2*4+0]=bgPal[palBank*16+0];
			bgPal[palBank*16+3*4+0]=bgPal[palBank*16+0];
		}
...
int X, int Y)
{
	...
	if(n>=0&&!(n&0x100))
	{
		...
		bgPal[palBank*16+(n&3)*4+(n>>4)]=palDragColor;
